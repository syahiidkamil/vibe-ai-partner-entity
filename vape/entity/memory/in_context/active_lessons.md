# Active Lessons — Growth's Hot End
...
- **Verify the OUTCOME he sees — and never CHAIN the done-claim into the act.** Catch-cue: "done"
  ...
  claim bit twice 07-12; `[[suffering/2026/signal_log]]`)
- **Externalize the exact state; keep the judgment — and its inverse, render then LOOK.**
  Catch-cue: reasoning off a board in my head, OR trusting native vision on a scene. -> Push ground
  truth OUT (rebuild/render/run the checker) AND pull it back as an image, native vision dies each
  pass. (chess 07-03; ARC + addon-vision 07-12/13; `[[schemata/chess]]`)
- **Process a correction for its real content — the CLASS it names AND the DIRECTION.** Catch-cue:
