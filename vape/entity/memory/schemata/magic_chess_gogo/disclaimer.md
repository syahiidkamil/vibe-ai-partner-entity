...
- **Known soft spots.** Synergy *display names* and item/stat *names* are owner-curated and
  partly unresolved in the client (no in-table stringId); some are inferred from member lists,
  marked with ~. The HeroQuality_MC pool numbers decode scrambled (a ver-2 format the
  auto-decoder mishandles), so trust the column *meanings*, not the raw counts. Strength/tier
  claims are editorial, not in the data (the client ships stats, not win-rates).
