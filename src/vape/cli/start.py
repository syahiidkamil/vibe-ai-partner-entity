...
from vape.cli._config import get_avatar_selection, get_port, read_config
from vape.cli._paths import RENDERERS_DIR, SHELLS_DIR, cache_dir
...
def _launch_avatar(renderer: str, shell: str, port: int) -> subprocess.Popen | None:
    ...
    if plugin is None:
        console.print(
            ...
            f"  [dim]Run `vape setup` to install it, or check plugins/renderers/{renderer}.[/dim]"
        )
        return None
