...
The six internal states consciousness monitors: **confidence**, **contextSaturation**, **alignment**, **memoryPressure**, **momentum**, **trustCalibration**. See [04-entity-model](04-entity-model.md) for definitions and feeling derivation formulas.
...
**Agent implementation**: Pillars 1-2 are implemented by the `consciousness-observer` agent (inherit model). Pillar 3 is implemented by the `free-will-deliberation` agent (inherit model). See [13-sub-agent-architecture](13-sub-agent-architecture.md) for agent details.
...
class ConsciousnessSystem {
  ...
  // Called on significant decision points
  // Runs the Free Will Protocol and returns the chosen response
  deliberate(event: HookEvent, context: string): FreeWillDeliberation;
...
interface FreeWillDeliberation {
  event: HookEvent;
  defaultResponse: string;                 // What conditioning says (cognitive decision)
  contrarian: string;                      // The opposite — challenges the assumption
  alternatives: string[];                  // Other options beyond default and contrarian
  chosenResponse: string;                  // What Free Will selected
