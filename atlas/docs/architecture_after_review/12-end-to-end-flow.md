...
### Hooks and Loops — Reactive + Proactive

The entity stays alive through two complementary mechanisms:
- **Hooks** (reactive): Fire on Claude Code events — tool use, prompts, responses. The entity reacts to what's happening.
- **Loops** (proactive): Fire on schedule — idle behaviors, mood decay, temporal checks. The entity maintains presence between interactions.

Together, hooks and loops create continuous life: the entity reacts when you work and stays present when you pause.
