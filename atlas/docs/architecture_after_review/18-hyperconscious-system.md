...
Hyperconsciousness can be activated three ways:

1. **User configures** — Set `ENTITY_HYPER_CONSCIOUSNESS_MODE=true` in `.env` for always-on
2. **Entity requests** — The consciousness-observer detects repeated conditioning overrides and requests deeper deliberation via `<conscious>` tag
3. **Slash command** — `/hyperconscious on` / `/hyperconscious off` for per-session toggle

The recommended approach is user-configured — turn it on for complex sessions (architecture, unfamiliar domains), turn it off for routine coding.
