...
Each model download includes a `capabilities.json` manifest declaring which feelings, expressions, and lip sync methods the model supports. The plugin reads this at load time — no hardcoded mappings. See [01-plugin-system](01-plugin-system.md) for the manifest schema.
