...
1. Read vape/entity/memory/conversations/{today's date}-session.md — the curated conversation log (primary input). If it doesn't exist, read recent git log for context.
2. Read vape/entity/state/current.json for final state snapshot.
3. Read vape/entity/consciousness/choices.md for any conscious decisions (if file exists).
4. Read vape/entity/consciousness/observations.md for self-observations (if file exists).
...
6. Append summary to vape/entity/memory/conversations/{today's date}-session.md under a "## Session Summary" heading.
