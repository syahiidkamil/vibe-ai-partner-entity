...
### Memory Mode → Plugin Mapping

| `.env` MEMORY_MODE | What's active | Plugin required |
|--------------------|--------------|-----------------|
| `basic` (default) | File-based only (`entity/memory/`, `entity/state/`) | None |
| `stateful` | File + PostgreSQL (state history, feeling timeline) | `plugin-memory/postgresql` |
| `intelligent` | File + PostgreSQL + pgvector + Gemini embeddings | `plugin-memory/postgresql` + `plugin-memory/pgvector` |

SQLite (`plugin-memory/sqlite`) is an alternative to PostgreSQL for `stateful` mode — lighter, no server required. Set via `DATABASE_URL=sqlite:///path/to/db.sqlite`.
...
**State decay formula** (exponential):
```
decayed = baseline + (current - baseline) × 0.5^(hoursInactive)
```

Example: confidence is 85, baseline is 50, after 1 hour inactive:
- `50 + (85 - 50) × 0.5^1 = 50 + 17.5 = 67.5`

After 2 hours: `50 + (85 - 50) × 0.5^2 = 50 + 8.75 = 58.75`

Decay is applied once at session start based on time elapsed since last state save, not continuously during a session.
