...
{
  "adjustments": [
    { "state": "confidence", "delta": +5 },
    { "state": "momentum", "delta": -3 }
  ]
}

Response: 200 OK
{
  "states": { "confidence": 65, "momentum": 47, ... },
  "feelings": { "happy": 72, "frustrated": 15, ... },
  "expressionsTriggered": ["nod"]
}
```

This is the primary endpoint for hooks to adjust internal states. The server applies deltas, runs the FeelingEngine, checks expression thresholds, and returns the full updated state. See [10-hooks-system](10-hooks-system.md) for the state adjustment mapping table.
